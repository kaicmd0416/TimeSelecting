...
class data_processing:
    ...
    def stock_highLow(self):
        ...
        # 计算过去三个月的最高价和最低价
        high_3m = df_stock.rolling(window='90D', min_periods=1).max()
        low_3m = df_stock.rolling(window='90D', min_periods=1).min()
        ...
        for col in df_stock.columns:
            ...
            # 获取历史最高价和最低价
            hist_high = high_3m[col].shift(1)  # 使用shift(1)排除当天
            hist_low = low_3m[col].shift(1)
    ...
    def targetIndex_RSI(self):
        ...
        #df=df[['valuation_date','RSI','target_index']]
        return df
